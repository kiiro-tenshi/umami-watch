    ...
    <div className="mt-6 border border-border bg-surface rounded-xl overflow-hidden shadow-sm">
      <div className="bg-surface-raised p-4 border-b border-border flex justify-between items-center">
        ...
        {totalPages > 1 && (
          <select value={page} onChange={(e) => setPage(Number(e.target.value))} 
            className="bg-surface border border-border rounded text-sm p-1.5 focus:outline-none focus:ring-1 focus:ring-accent-teal text-secondary font-medium outline-none">
      ...
      <div className="max-h-96 overflow-y-auto divide-y divide-border-subtle">
        {currentEpisodes.map((ep) => {
          ...
          return (
            <div key={ep.id} className={`p-4 flex items-center justify-between hover:bg-surface-raised transition-colors ${isCurrent ? 'bg-red-50/50' : ''}`}>
              ...
              <Link to={`/watch?type=anime&animeId=${animeId}&aniwatchEpisodeId=${encodeURIComponent(ep.id)}&epNum=${ep.number}${roomId ? `&roomId=${roomId}` : ''}`}
                className={`shrink-0 px-4 py-1.5 rounded-lg text-sm font-bold shadow-sm transition-transform hover:scale-105 ${isCurrent ? 'bg-accent-teal text-white' : 'bg-accent-blue hover:bg-red-700 text-white'}`}>
