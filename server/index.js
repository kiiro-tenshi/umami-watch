...
app.get('/api/proxy/hls', async (req, res) => {
  ...
  const targetReferer = decodeURIComponent(referer || 'https://hianime.to/');
  ...
  try {
    ...
    if (isM3u8) {
      ...
      const makeProxied = (rawUrl) => {
        const abs = rawUrl.startsWith('http')
          ...
          : rawUrl.startsWith('/')
            ...
            : basePath + rawUrl;
        return `/api/proxy/hls?url=${encodeURIComponent(abs)}&referer=${encodeURIComponent(targetReferer)}`;
      };

      const rewritten = text.split('\n').map(line => {
        const trimmed = line.trim();
        if (trimmed === '') return line;
        // Rewrite URI="..." in EXT-X-KEY, EXT-X-MEDIA, etc.
        if (trimmed.startsWith('#') && trimmed.includes('URI="')) {
          return line.replace(/URI="([^"]+)"/, (_, uri) => `URI="${makeProxied(uri)}"`);
        }
        // Skip other directives
        if (trimmed.startsWith('#')) return line;
        // It's a segment/variant URL
      ...
      // Proxy binary content (TS segments, VTT subtitles, etc.)
